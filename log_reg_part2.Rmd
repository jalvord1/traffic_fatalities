...
```{r}
accident1$persons_n <- ifelse(accident1$total_persons == 1, "one person involved", 
                              ifelse(accident1$total_persons == 2, "two people involved", "more than two people involved"))

ggplot(accident1, aes(percent_killed)) + geom_bar(aes(fill=persons_n))

table(accident1$persons_n, accident1$percent_killed)
...
#to look at the predictors individually
  ...
  glm_probs <- data.frame(probs = predict(glm_fit,
                                       newdata = test,
                                       type="response"))
  #if probs is above 50% then prediction is above else below
  glm_pred <- glm_probs %>%
    mutate(pred = ifelse(probs>.5, "above", "below"))
  
  glm_pred <- cbind(test, glm_pred)
  
  #how often was it correct overall
  result <- glm_pred %>%
    summarize(score = mean(pred == above_fatal_med))
  ...
  #our model is much worse at predicting accidents as above when they actually are
  #misclassified as below when it was actually above 72% of the time
```

#New variables
```{r}
accident_small <- accident_final %>%
  select(-c(weather_cat, work_zone_bin, months_cat, type_int_cat, hour_cat, states_cat, lgt_cond_cat)) %>%
  mutate(id = rownames(accident_small))

#rerunning model
set.seed(10)
  
  train <- accident_small %>%
    sample_frac(0.2)
...
```{r}
accident_final <- accident_test %>%
  #getting rid of the variables used to create y variable
  select(-c(total_persons)) %>%
  #filtering out certain levels of the factors in order to avoid error on predict function
  #should we do bootstrap instead?
  filter(!states_cat %in% c("District of Columbia", "Puerto Rico"),
         !man_coll_cat == "rear-to-rear",
...
accident_small <- accident_final %>%
  select(-c(weather_cat, work_zone_bin, months_cat, type_int_cat, hour_cat, states_cat, lgt_cond_cat)) %>%
  mutate(id = rownames(accident_final))

fatal_med <- median(accident_small$percent_killed)

accident_small <- accident_small %>%
  mutate(above_fatal_med = ifelse(percent_killed >= fatal_med, "above", "below")) %>%
  select(-percent_killed)
